...
public class GlyphBitmapIconLoader extends SwingWorker<GlyphBitmapIcon, Void> {
    ...
    private static final Logger LOGGER = Logger.getLogger(GlyphBitmapIconLoader.class
            .getName());
    ...
    public GlyphBitmapIconLoader(GlyphDefinition glyphDefinition, JComponent container, int size) {
    ...
    public GlyphBitmapIcon doInBackground() throws IOException {
        BufferedImage bi = loadImage(glyphDefinition.getDataSource().getBasePath(),
                glyphDefinition.getUrl());
        if (bi != null) {
            return new GlyphBitmapIcon(scaleToBound(bi, size, size),
                    size);
        }
        return null;
    }
