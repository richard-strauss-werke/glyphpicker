 ...
 * <ol>
 * <li>typing any value into the editor when the popup is invisible causes the
 * popup to appear and its contents to be filtered according to the editor's
 * text. It also autocompletes to the first item that is prefixed with the
 * editor's text and selects that item within the popup.</li>
 * <li>typing any value into the editor when the popup is visible causes the
 * popup to be refiltered according to the editor's text and reselects an
 * appropriate autocompletion item.</li>
 ...
 * according to the editor's text. It also autocompletes to the first item that
 * is prefixed with the editor's text and selects that item within the popup.</li>
 * <li>typing the up arrow key when the popup is visible and the selected
 * element is the first element causes the autocompletion to be cleared and the
 * popup's selection to be removed.</li>
 * <li>typing the up arrow key when no selection exists causes the last element
 * of the popup to become selected and used for autocompletion</li>
 * <li>typing the down arrow key when the popup is visible and the selected
 * element is the last element causes the autocompletion to be cleared and the
 * popup's selection to be removed</li>
 ...
 * <li>typing the delete key while in strict mode will select the prior text
 ...
 * appear and its contents to be shown unfiltered
 ...
 * {@link JComboBox#getPrototypeDisplayValue() prototype display value} if a
 * non-null prototype display value exists
 * <li>as items are filtered in the ComboBoxModel, the popup height is adjusted
 ...
 * <li>the selected item within the popup is changed (which can happen due to a
 * mouse click, a change in the autocompletion term, or using the arrow keys)
 ...
 * renderer, etc. By default, JComboBox relies on {@link Object#toString()} to
 * map elements to their String equivalents. Sometimes, however, toString() is
 * not a reliable or desirable mechanism to use. To deal with this problem,
 ...
 * the given items
 * <li>the ComboBoxEditor will be wrapped with functionality and set back into
 * the JComboBox as the editor
...
public final class CustomAutoCompleteSupport<E> {
    ...
    /**
     * <tt>true</tt> if the {@link #popupMenu} should <strong>always</strong> be
     * hidden when the {@link #comboBoxEditor} loses focus; <tt>false</tt> if
     * the default behaviour should be preserved. This exists to provide a
     ...
     * typically used to represent "no selection".
     */
    private final EventList<E> firstItem;
    /**
     * The CompositeList which is the union of firstItem and filteredItems to
     * produce all filtered items available in the comboBoxModel.
     */
    private final CompositeList<E> allItemsFiltered;
    /**
     * The CompositeList which is the union of firstItem and items to produce
     * all unfiltered items available in the comboBoxModel.
     */
    private final CompositeList<E> allItemsUnfiltered;
    /**
     * The Format capable of producing Strings from ComboBoxModel elements and
     * vice versa.
     */
     ...
     * computed using Object.toString(). (i.e. the default ComboBoxEditor *does*
     ...
     * {@link #comboBoxEditorComponent}; <tt>false</tt> otherwise.
     */
    private boolean isFiltering = false;
    /**
     * Controls the selection behavior of the JComboBox when it is used in a
     * JTable DefaultCellEditor.
     */
    private final boolean isTableCellEditor;
    //
    // These listeners work together to enforce different aspects of the
    // autocompletion behaviour
    //
    /**
     * The MouseListener which is installed on the {@link #arrowButton} and is
     * responsible for clearing the filter and then showing / hiding the
     * {@link #popup}.
     */
    private ArrowButtonMouseListener arrowButtonMouseListener;
    /**
     * A listener which reacts to changes in the ComboBoxModel by resizing the
     * popup appropriately to accomodate the new data.
     */
     ...
     * Namely, we respect the prototype display value of the combo box, if it
     * has one. Regardless of the width of the combo box, we attempt to size the
     * popup to accomodate the width of the prototype display value.
     */
     ...
     * attempts to autocomplete for fear that we will replace the user's newly
    ...
    /**
     * Handles the special case of the backspace key in strict mode and the
     * enter key.
     */
    ...
    //
    // These listeners watch for invalid changes to the JComboBox which break
    // our autocompletion
    //
    /**
     * Watches for changes of the Document which backs comboBoxEditorComponent
     * and uninstalls our DocumentFilter from the old Document and reinstalls it
     * on the new.
     */
    ...
    //
    // These booleans control when certain changes are to be respected and when
    // they aren't
    //
    /**
     * <tt>true</tt> indicates document changes should not be post processed
     * (i.e. just commit changes to the Document and do not cause any
     * side-effects).
     */
    private boolean doNotPostProcessDocumentChanges = false;
    /**
     * <tt>true</tt> indicates attempts to filter the ComboBoxModel should be
     * ignored.
     */
    ...
    /**
     * <tt>true</tt> indicates attempts to select an autocompletion term should
     * be ignored.
     */
    private boolean doNotAutoComplete = false;
    /**
     * <tt>true</tt> indicates attempts to toggle the state of the popup should
     ...
     * opposed to typed into the JComboBox directly)
     */
     ...
     * <code>filterator</code> is consulted in order to extract searchable text
     * from each of the <code>items</code>. Non-null <code>format</code> objects
    ...
    private CustomAutoCompleteSupport(JComboBox comboBox, EventList<E> items,
        ...
        this.items = items;
        this.format = format;
        // only build a custom renderer if the user specified their own Format
        // but has not installed a custom renderer of their own
        final boolean defaultRendererInstalled = comboBox.getRenderer() instanceof UIResource;
        this.renderer = format != null && defaultRendererInstalled ? new StringFunctionRenderer()
                : null;
        // is this combo box a TableCellEditor?
        this.isTableCellEditor = Boolean.TRUE.equals(comboBox
        ...
        try {
            ...
            this.filterMatcherEditor = new TextMatcherEditor(
                    filterator == null ? new DefaultTextFilterator()
                            : filterator);
            this.filterMatcherEditor.setMode(TextMatcherEditor.STARTS_WITH);
            this.filteredItems = new FilterList<E>(items,
                    this.filterMatcherEditor);
            this.firstItem = new BasicEventList<E>(items.getPublisher(),
                    items.getReadWriteLock());
            // the ComboBoxModel always contains the firstItem and a filtered
            // view of all other items
            this.allItemsFiltered = new CompositeList<E>(items.getPublisher(),
                    items.getReadWriteLock());
            this.allItemsFiltered.addMemberList(this.firstItem);
            this.allItemsFiltered.addMemberList(this.filteredItems);
            this.comboBoxModel = new AutoCompleteComboBoxModel(
                    this.allItemsFiltered);
            // we need an unfiltered view in order to try to locate
            // autocompletion terms
            this.allItemsUnfiltered = new CompositeList<E>(
                    items.getPublisher(), items.getReadWriteLock());
        ...
        decorateCurrentUI();
        // react to changes made to the key parts of JComboBox which affect
        // autocompletion
        this.comboBox.addPropertyChangeListener("UI", this.uiWatcher);
        this.comboBox.addPropertyChangeListener("model", this.modelWatcher);
        this.comboBoxEditorComponent.addPropertyChangeListener("document",
                this.documentWatcher);
    }

    /**
     * A convenience method to ensure {@link CustomAutoCompleteSupport} is being
     * accessed from the Event Dispatch Thread.
     */
    private static void checkAccessThread() {
        if (!SwingUtilities.isEventDispatchThread())
            throw new IllegalStateException(
                    "AutoCompleteSupport must be accessed from the Swing Event Dispatch Thread, but was called on Thread \""
    ...
    private void decorateCurrentUI() {
        ...
        this.popupMenu = (JPopupMenu) comboBox.getUI().getAccessibleChild(
                comboBox, 0);
        ...
        // with logic that unfilters the ComboBoxModel when the arrow button is
        // pressed
        if (this.arrowButton != null) {
            this.arrowButton.removeMouseListener(popup.getMouseListener());
            this.arrowButtonMouseListener = new ArrowButtonMouseListener(
                    popup.getMouseListener());
            this.arrowButton.addMouseListener(arrowButtonMouseListener);
        }
        // start listening for model changes (due to filtering) so we can resize
        // the popup vertically
        this.comboBox.getModel().addListDataListener(listDataHandler);
        // calculate the popup's width according to the prototype value, if one
        // exists
        ...
        // start suppressing autocompletion when selecting values from the popup
        // with the mouse
        ...
        this.originalAquaSelectPreviousAction = actionMap
                .get("aquaSelectPrevious");
        ...
        // install a custom ComboBoxEditor that decorates the existing one, but
        // uses the
        // convertToString(...) method to produce text for the editor component
        ...
        this.comboBoxEditorComponent = (JTextField) comboBox.getEditor()
                .getEditorComponent();
        this.document = (AbstractDocument) comboBoxEditorComponent
                .getDocument();
        this.document.setDocumentFilter(documentFilter);
        // install a custom renderer on the combobox, if we have built one
        if (this.renderer != null)
            comboBox.setRenderer(renderer);
        // add a KeyListener to the ComboBoxEditor which handles the special
        // case of backspace when in strict mode
        ...
        // focus is gained
        this.comboBoxEditorComponent
                .addFocusListener(selectTextOnFocusGainHandler);
    }

    /**
     * Remove all customizations installed to various areas of the current UI in
    ...
    private void undecorateOriginalUI() {
        ...
        // remove the DocumentFilter from the Document backing the editor
        // JTextField
        this.document.setDocumentFilter(null);
        // restore the original ComboBoxEditor if our custom ComboBoxEditor is
        // still installed
        ...
        // stop suppressing autocompletion when selecting values from the popup
        // with the mouse
        ...
        // remove the KeyListener from the ComboBoxEditor which handles the
        // special case of backspace when in strict mode
        this.comboBoxEditorComponent
                .removeKeyListener(strictModeBackspaceHandler);
        // remove the FocusListener from the ComboBoxEditor which selects all
        // text when focus is gained
        this.comboBoxEditorComponent
                .removeFocusListener(selectTextOnFocusGainHandler);
    ...
    public static <E> CustomAutoCompleteSupport<E> install(JComboBox comboBox,
        ...
        if (comboBox.getModel().getClass() == CustomAutoCompleteSupport.AutoCompleteComboBoxModel.class)
            throw new IllegalArgumentException(
                    "comboBox is already configured for autocompletion");
        return new CustomAutoCompleteSupport<E>(comboBox, items, filterator,
                format);
    }

    /**
     * This method is used to report environmental invariants which are violated
     * when the user adjusts the combo box in a way that is incompatible with
     * the requirements for autocompletion. A message can be specified which
    ...
    /**
     * Returns <tt>true</tt> if user specified strings are converted to the case
     * of the autocompletion term they match; <tt>false</tt> otherwise.
     */
    ...
    /**
     * If <code>strict</code> is <tt>false</tt>, the user can specify values not
    ...
    public void setStrict(boolean strict) {
        ...
        if (strict) {
            ...
            boolean textMatches = GlazedListsImpl.equal(currentItemText,
                    currentText);
            ...
            if (currentItem == NOT_FOUND && !allItemsUnfiltered.isEmpty()) {
                ...
                textMatches = GlazedListsImpl.equal(currentItemText,
                        currentText);
            }
    ...
    /**
     * Returns <tt>true</tt> if a beep sound is played when the user attempts to
     * violate the strict invariant; <tt>false</tt> if no beep sound is played.
     * This setting is only respected if {@link #isStrict()} returns
     * <tt>true</tt>.
     *
     * @see #setStrict(boolean)
     */
    ...
    /**
     * Returns <tt>true</tt> if the popup menu is hidden whenever the combo box
     * editor loses focus; <tt>false</tt> otherwise.
     */
    ...
    public void setFilterMode(int mode) {
        checkAccessThread();
        // adjust the MatcherEditor that filters the AutoCompleteComboBoxModel
        // to respect the given mode
        // but ONLY adjust the contents of the model, avoid changing the text in
        // the JComboBox's textfield
    ...
    /**
     * Sets the manner in which the contents of the {@link ComboBoxModel} are
     * filtered and autocompletion terms are matched. The given
     * <code>strategy</code> must be one of
    ...
    /**
     * Returns the manner in which the contents of the {@link ComboBoxModel} are
     * filtered and autocompletion terms are matched. The returned
     * <code>strategy</code> is one of
     * {@link TextMatcherEditor#IDENTICAL_STRATEGY} or
     * {@link TextMatcherEditor#NORMALIZED_STRATEGY} or the Unicode strategy of
     * the ICU4J extension.
     */
    ...
    /**
     * This method removes autocompletion support from the {@link JComboBox} it
     * was installed on. This method is useful when the {@link EventList} of
     * items that backs the combo box must outlive the combo box itself. Calling
     * this method will return the combo box to its original state before
    ...
    public void uninstall() {
        ...
        if (this.comboBox == null)
            throw new IllegalStateException(
                    "This AutoCompleteSupport has already been uninstalled");
        items.getReadWriteLock().readLock().lock();
        try {
            // 1. stop listening for changes
            this.comboBox.removePropertyChangeListener("UI", this.uiWatcher);
            this.comboBox.removePropertyChangeListener("model",
                    this.modelWatcher);
            this.comboBoxEditorComponent.removePropertyChangeListener(
                    "document", this.documentWatcher);
            ...
            // 6. dispose of our EventLists so that they are severed from the
            // given items EventList
            this.allItemsFiltered.dispose();
            this.allItemsUnfiltered.dispose();
            this.filteredItems.dispose();
            // null out the comboBox to indicate that this support class is
            // uninstalled
            this.comboBox = null;
        } finally {
            items.getReadWriteLock().readLock().unlock();
    ...
    private void togglePopup() {
        // break out early if we're flagged to ignore attempts to toggle the
        // popup state
        if (doNotTogglePopup)
            return;
     ...
     * side-effect of setting the selected item. It also marks another flag to
     * indicate that the selected item is being explicitly set, and thus
     * autocompletion should not execute and possibly overwrite the programmer's
     * specified value.
     */
    private class AutoCompleteComboBoxModel extends EventComboBoxModel<E> {
        ...
        public void setSelectedItem(Object selected) {
            ...
            // remove all ActionListeners from the JComboBox since setting the
            // selected item
            // would normally notify them, but in normal autocompletion
            // behaviour, we don't want that
            final ActionListener[] listeners = unregisterAllActionListeners(comboBox);
            try {
                super.setSelectedItem(selected);
                if (comboBoxEditorComponent != null) {
                    // remove any text selection that might exist when an item
                    // is selected
                    final int caretPos = comboBoxEditorComponent
                            .getCaretPosition();
         ...
         * the text of the selected item. We don't want that.
         * AutoCompleteSupport is the ultimate authority on the text value in
         * the ComboBoxEditor. We override this method to set
         * doNotChangeDocument to ensure that attempts to change the
         * ComboBoxEditor's Document are ignored and our control is absolute.
         */
        @Override
     ...
     * ComboBoxEditor component. It is our hook to either control when to
     * respect edits as well as the side-effects the edit has on autocompletion
     * and filtering.
     */
    private class AutoCompleteFilter extends DocumentFilter {
        ...
        public void replace(FilterBypass filterBypass, int offset, int length,
            ...
            final int selectionStart = comboBoxEditorComponent
                    .getSelectionStart();
            final int selectionEnd = comboBoxEditorComponent.getSelectionEnd();
            // this short-circuit corrects the PlasticLookAndFeel behaviour.
            // Hitting the enter key in Plastic
            // will cause the popup to reopen because the Plastic ComboBoxEditor
            // forwards on unnecessary updates
            // to the document, including ones where the text isn't really
            // changing
            final boolean isReplacingAllText = offset == 0
            ...
            postProcessDocumentChange(filterBypass, attributeSet,
                    valueBeforeEdit, selectionStart, selectionEnd, true);
        }
        ...
        public void remove(FilterBypass filterBypass, int offset, int length)
            ...
            postProcessDocumentChange(filterBypass, null, valueBeforeEdit,
                    selectionStart, selectionEnd, isStrict());
        }
         ...
         * <li>update the text in the combo box editor with an autocomplete
         * suggestion
        ...
        private void postProcessDocumentChange(FilterBypass filterBypass,
            ...
            // if an autocomplete term could not be found and we're in strict
            // mode, rollback the edit
            if (isStrict()
                    && (findAutoCompleteTerm(valueAfterEdit) == NOT_FOUND)
                    && !allItemsUnfiltered.isEmpty()) {
        ...
        private void selectAutoCompleteTerm(FilterBypass filterBypass,
            ...
            if (doNotAutoComplete)
                return;
            // determine if our prefix is empty (in which case we cannot use our
            // filterMatcher to locate an autocompletion term)
            final boolean prefixIsEmpty = "".equals(prefix);
            // record the original caret position in case we don't want to
            // disturb the text (occurs when an exact autocomplete term match is
            // found)
            final int originalCaretPosition = comboBoxEditorComponent
                    .getCaretPosition();
            // a flag to indicate whether a partial match or exact match exists
            // on the autocomplete term
            boolean autoCompleteTermIsExactMatch = false;
            // search the combobox model for a value that starts with our prefix
            // (called an autocompletion term)
            for (int i = 0, n = comboBoxModel.getSize(); i < n; i++) {
                String itemString = convertToString(comboBoxModel
                        .getElementAt(i));
                // if itemString does not match the prefix, continue searching
                // for an autocompletion term
                if (prefixIsEmpty ? !"".equals(itemString) : !filterMatcher
                        .matches(itemString))
                    continue;
                // record the index and value that are our "best" autocomplete
                // terms so far
                int matchIndex = i;
                String matchString = itemString;
                // search for an *exact* match in the remainder of the
                // ComboBoxModel
                // before settling for the partial match we have just found
                for (int j = i; j < n; j++) {
                    itemString = convertToString(comboBoxModel.getElementAt(j));
                    // if we've located an exact match, use its index and value
                    // rather than the partial match
                ...
                // if partial autocompletion terms are not allowed, and we only
                // have a partial term, bail early
                if (!allowPartialAutoCompletionTerm
                        && !prefix.equals(itemString))
                    return;
                // either keep the user's prefix or replace it with the
                // itemString's prefix
                // depending on whether we correct the case
                if (getCorrectsCase() || isStrict()) {
                    filterBypass.replace(0, prefix.length(), matchString,
                            attributeSet);
                } else {
                    final String itemSuffix = matchString.substring(prefix
                            .length());
                    filterBypass.insertString(prefix.length(), itemSuffix,
                            attributeSet);
                }
                ...
                if (autoCompleteTermIsExactMatch) {
                    ...
                    // caret to its original location
                    comboBoxEditorComponent
                            .setCaretPosition(originalCaretPosition);
                } else {
                    // select the text after the prefix but before the end of
                    // the text (it represents the autocomplete text)
                    comboBoxEditorComponent.select(prefix.length(),
                            document.getLength());
                }
                return;
            }
            // reset the selection since we couldn't find the prefix in the
            // model
            // (this has the side-effect of scrolling the popup to the top)
            final boolean silently = isTableCellEditor
                    || selectedItemBeforeEdit == null;
        ...
        private void selectItem(int index, boolean silently) {
            final Object valueToSelect = index == -1 ? null : comboBoxModel
                    .getElementAt(index);
            // if nothing is changing about the selection, return immediately
            if (GlazedListsImpl.equal(comboBoxModel.getSelectedItem(),
                    valueToSelect))
                return;
            doNotChangeDocument = true;
            try {
    ...
    private void selectPossibleValue(int index) {
        ...
        final boolean validIndex = index >= 0
                && index < comboBox.getModel().getSize();
        // if the index isn't valid, select nothing
        if (!validIndex)
            index = -1;
        // adjust only the value in the comboBoxEditorComponent, but leave the
        // comboBoxModel unchanged
        doNotPostProcessDocumentChanges = true;
        try {
            // select the index
            if (isTableCellEditor) {
                // while operating as a TableCellEditor, no ActionListeners must
                // be notified
            ...
            if (!validIndex) {
                ...
                // don't bother unfiltering the popup since we'll redisplay the
                // popup immediately
        ...
        // if the comboBoxEditorComponent's values begins with the user's
        // prefix, highlight the remainder of the value
    ...
    /**
     * This class listens to the ComboBoxModel and redraws the popup if it must
     * grow or shrink to accomodate the latest list of items.
     */
    private class ListDataHandler implements ListDataListener {
        ...
        public void contentsChanged(ListDataEvent e) {
            ...
            // if the size of the model didn't change, the popup size won't
            // change
            if (previousItemCount != newItemCount) {
                ...
                if (popupMenu.isShowing()) {
                    if (comboBox.isShowing()) {
                        // if either the previous or new item count is less than
                        // the max,
                        // hide and show the popup to recalculate its new height
                        if (newItemCount < maxPopupItemCount
                                || previousItemCount < maxPopupItemCount) {
                            // don't bother unfiltering the popup since we'll
                            // redisplay the popup immediately
                            doNotClearFilterOnPopupHide = true;
                            try {
                                comboBox.hidePopup();
                            } finally {
                        ...
                        // if the comboBox is not showing, simply hide the popup
                        // to avoid:
                        // "java.awt.IllegalComponentStateException: component must be showing on the screen to determine its location"
                        // this case can occur when the comboBox is used as a
                        // TableCellEditor
                        // and is uninstalled (removed from the component
                        // hierarchy) before
            ...
            // if the comboBoxModel was changed and it wasn't due to the filter
            // changing
            // (i.e. !isFiltering) and it wasn't because the user selected a new
            // selectedItem (i.e. !userSelectedNewItem) then those changes may
            // have
            // invalidated the invariant that strict mode places on the text in
            // the
            // JTextField, so we must either:
            //
            // a) locate the text within the model (proving that the strict mode
            // invariant still holds)
            // b) set the text to that of the first element in the model (to
            // reestablish the invariant)
            final boolean userSelectedNewItem = e.getIndex0() == -1
                    || e.getIndex1() == -1;
            if (isStrict() && !userSelectedNewItem && !isFiltering) {
                // notice that instead of doing the work directly, we post a
                // Runnable here
                // to check the strict mode invariant and repair it if it is
                // broken. That's
                // important. It's necessary because we must let the current
                // ListEvent
                // finish its dispatching before we attempt to change the filter
                // of the
                // filteredItems list by setting new text into the
                // comboBoxEditorComponent
            ...
            public void run() {
                ...
                if (editor != null) {
                    ...
                    if (!currentText.equals(itemText)) {
                        // select the first item if we could not find an
                        // autocomplete term with the currentText
                        if (item == NOT_FOUND && !allItemsUnfiltered.isEmpty())
                            itemText = convertToString(allItemsUnfiltered
                                    .get(0));
                        // set the new strict value text into the editor
                        // component
    ...
    /**
     * This class sizes the popup menu of the combo box immediately before it is
     * shown on the screen. In particular, it will adjust the width of the popup
     * to accomodate a prototype display value if the combo box contains one.
     */
    private class PopupSizer implements PopupMenuListener {
        @Override
        public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
            // if the combo box does not contain a prototype display value, skip
            // our sizing logic
            ...
            if (popupComponent.getComponent(0) instanceof JScrollPane) {
                final JScrollPane scroller = (JScrollPane) popupComponent
                        .getComponent(0);
                // fetch the existing preferred size of the scroller, and we'll
                // check if it is large enough
                final Dimension scrollerSize = scroller.getPreferredSize();
                // calculates the preferred size of the renderer's component for
                // the prototype value
                final Dimension prototypeSize = getPrototypeSize(prototypeValue);
                // add to the preferred width, the width of the vertical
                // scrollbar, when it is visible
                prototypeSize.width += scroller.getVerticalScrollBar()
        ...
        public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
            if (doNotClearFilterOnPopupHide)
                return;
            // the popup menu is being hidden, so clear the filter to return the
            // ComboBoxModel to its unfiltered state
            applyFilter("");
        }
    ...
    /**
     * When the user clicks on the arrow button, we always clear the filtering
     * from the model to emulate Firefox style autocompletion.
     */
     ...
     * and the {@link CustomAutoCompleteSupport} is strict. Normally backspace
     * would delete the selected text, if it existed, or delete the character
     ...
     * <strong>NEVER</strong> alters the Document. Rather, it alters the text
     * selection to include one more character to the left. This is a nice
     ...
     * enter key is detected we actually unregister all ActionListeners, process
     * the keystroke as normal, then reregister the listeners and broadcast an
     * event to them, producing a single ActionEvent.
     */
    private class AutoCompleteKeyHandler extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            // this KeyHandler performs ALL processing of the ENTER key
            // otherwise multiple
            // ActionEvents are fired to ActionListeners by the default
            // JComboBox processing.
            // To control processing of the enter key, we set a flag to avoid
            // changing the
            // editor's Document in any way, and also unregister the
            // ActionListeners temporarily.
            if (e.getKeyChar() == KeyEvent.VK_ENTER) {
                doNotChangeDocument = true;
                this.actionListeners = unregisterAllActionListeners(comboBox);
            }
            // make sure this backspace key does not modify our
            // comboBoxEditorComponent's Document
            if (isTrigger(e))
                doNotChangeDocument = true;
        }
        ...
        public void keyTyped(KeyEvent e) {
            if (isTrigger(e)) {
                // if no content exists in the comboBoxEditorComponent, bail
                // early
                if (comboBoxEditorComponent.getText().length() == 0)
                    return;
                // calculate the current beginning of the selection
                int selectionStart = Math.min(
                        comboBoxEditorComponent.getSelectionStart(),
                        comboBoxEditorComponent.getSelectionEnd());
                // if we cannot extend the selection to the left, indicate the
                // error
                if (selectionStart == 0) {
                    if (getBeepOnStrictViolation())
                        UIManager.getLookAndFeel().provideErrorFeedback(
                ...
                selectionStart--;
                // select the text from the end of the Document to the new
                // selectionStart
                // (which positions the caret at the selectionStart)
                comboBoxEditorComponent
                        .setCaretPosition(comboBoxEditorComponent.getText()
                                .length());
        ...
        public void keyReleased(KeyEvent e) {
            // resume the ability to modify our comboBoxEditorComponent's
            // Document
            if (isTrigger(e))
                doNotChangeDocument = false;
            // keyPressed(e) has disabled the JComboBox's normal processing of
            // the enter key
            // so now it is time to perform our own processing. We reattach all
            // ActionListeners
            // and simulate exactly ONE ActionEvent in the JComboBox and then
            // reenable Document changes.
            if (e.getKeyChar() == KeyEvent.VK_ENTER) {
                updateFilter();
                // reregister all ActionListeners and then notify them due to
                // the ENTER key
                ...
                // then the ENTER key transfers focus back to the
                // ComboBoxEditor.
                if (actionListeners != null) {
                    registerAllActionListeners(comboBox, actionListeners);
                    comboBox.actionPerformed(new ActionEvent(e.getSource(), e
                            .getID(), null));
                }
     ...
     * returned from
     * {@link CustomAutoCompleteSupport#getSelectsTextOnFocusGain()} allows this
     * behaviour. In addition, the JPopupMenu is hidden when the ComboBoxEditor
     * loses focus if the value returned from
     * {@link CustomAutoCompleteSupport#getHidesPopupOnFocusLost()} allows this
     * behaviour.
     */
    private class ComboBoxEditorFocusHandler extends FocusAdapter {
        ...
        public void focusGained(FocusEvent e) {
            if (getSelectsTextOnFocusGain())
                comboBoxEditorComponent.select(0, comboBoxEditorComponent
                        .getText().length());
        }
    ...
    private class ModelWatcher implements PropertyChangeListener {
        @Override
        public void propertyChange(PropertyChangeEvent evt) {
            throwIllegalStateException("The ComboBoxModel cannot be changed. It was changed to: "
                    + evt.getNewValue());
        }
    }
    ...
    private class DocumentWatcher implements PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if (!(newDocument instanceof AbstractDocument))
                throwIllegalStateException("The Document behind the JTextField was changed to no longer be an AbstractDocument. It was changed to: "
                        + newDocument);
     ...
     * delegate by installing a new FormatComboBoxEditor overtop of the UI
     * Delegate's default ComboBoxEditor.
     */
    private class FormatComboBoxEditor implements ComboBoxEditor, UIResource {
        /**
         * This is the ComboBoxEditor installed by the current UI Delegate of
         * the JComboBox.
         */
        ...
        public void setItem(Object anObject) {
            ...
            ((JTextField) getEditorComponent())
                    .setText(convertToString(anObject));
        }

        /**
         * BasicComboBoxEditor defines this method to use reflection to try
         * finding a method called valueOf(String) in order to return the item.
         * We attempt to find a user-supplied Format before resorting to the
         * valueOf(String) call.
         */
        @Override
        public Object getItem() {
            final String oldValueString = convertToString(oldValue);
            final String currentString = ((JTextField) getEditorComponent())
                    .getText();
            ...
            // otherwise, use the default algorithm from BasicComboBoxEditor to
            // produce a value
    ...
    /**
     * This default implementation of the TextFilterator interface uses the same
     * strategy for producing Strings from ComboBoxModel objects as the renderer
     * and editor.
     */
    ...
    public static class AutoCompleteCellEditor<E> extends DefaultCellEditor {
         ...
         * <code>autoCompleteSupport</code>. Specifically, the JComboBox is
         * retrieved using {@link CustomAutoCompleteSupport#getComboBox()}.
         */
        public AutoCompleteCellEditor(
                CustomAutoCompleteSupport<E> autoCompleteSupport) {
         ...
         */
        public CustomAutoCompleteSupport<E> getAutoCompleteSupport() {
    ...
    public static <E> AutoCompleteCellEditor<E> createTableCellEditor(
        ...
        return createTableCellEditor(GlazedLists.comparableComparator(),
                tableFormat, tableData, columnIndex);
    }
    ...
    public static <E> AutoCompleteCellEditor<E> createTableCellEditor(
        ...
        final CustomAutoCompleteSupport<E> autoCompleteSupport = CustomAutoCompleteSupport
                .install(comboBox, source);
        autoCompleteSupport.setSelectsTextOnFocusGain(false);
        // create an AutoCompleteCellEditor using the AutoCompleteSupport object
        final AutoCompleteCellEditor<E> cellEditor = new AutoCompleteCellEditor<E>(
                autoCompleteSupport);
    ...
    private static final class TableCellComboBox extends JComboBox implements
        ...
        public TableCellComboBox() {
            ...
            replaceUIDelegateFocusListener(getEditor().getEditorComponent(),
                    this);
         ...
         * focus implies we are committing or cancelling the cell edit anyway,
        ...
        public void focusGained(FocusEvent e) {
            ...
            if (currentEditor != null
                    && currentEditor.getEditorComponent() != e.getSource()) {
                repaint();
                if (isEditable()) {
        ...
        public void focusLost(FocusEvent e) {
            final ComboBoxEditor currentEditor = getEditor();
            if (!e.isTemporary() && currentEditor != null
                    && currentEditor.getEditorComponent() == e.getSource()) {
                final Object currentItem = currentEditor.getItem();
                if (currentItem != null
                        && !currentItem.equals(getSelectedItem())) {
        ...
        private void fireActionPerformed(ComboBoxEditor source) {
            actionPerformed(new ActionEvent(source, 0, "",
        ...
        protected boolean processKeyBinding(KeyStroke ks, KeyEvent e,
            ...
            // ensure the text field has focus if it is still processing key
            // strokes
            // (I've seen bad behaviour on windows textfield has no cursor, yet
            // continues to process keystrokes
            // - this helps to ensure that the textfield actually has focus and
            // thus the cursor)
            if (!tableCellTextField.hasFocus())
                tableCellTextField.requestFocus();
            // now let the JComboBox react (important for arrow keys to work as
            // expected)
        ...
        public void setNextFocusableComponent(Component aComponent) {
            ...
            ((JComponent) getEditor().getEditorComponent())
                    .setNextFocusableComponent(aComponent);
        }

        /**
         * A custom BasicComboBoxEditor that builds a custom JTextField with an
         * extra capability: a public implementation of
         * {@link TableCellTextField#processKeyBinding}
         */
        private static final class TableCellComboBoxEditor extends
                BasicComboBoxEditor {
            public TableCellComboBoxEditor() {
                // replace the super's editor with a JTextField of our own
                // design
        ...
        private static class TableCellTextField extends JTextField {
            ...
            private boolean equalsText(String newText) {
                ...
                return (currentText == null) ? newText == null : currentText
                        .equals(newText);
            }
     ...
     */
    private static final class TableColumnValueFunction<E> implements
            FunctionList.Function<E, Object> {
